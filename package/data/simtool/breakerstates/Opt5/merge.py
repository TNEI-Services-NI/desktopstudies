...
if __name__ == '__main__':
    ...
    csvs = list(filter(lambda x: ('.csv' in x)&('chapelcross33kv'!=x)&('merged.csv'!=x), files))
    ...
    df_concat.drop_duplicates(subset='breaker', keep='first').to_csv(os.getcwd() + '/merged.csv', index=False)
