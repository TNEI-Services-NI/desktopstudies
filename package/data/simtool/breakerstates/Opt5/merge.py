...
if __name__ == '__main__':
    ...
    [list_dfs.append(pd.read_csv(os.getcwd() + "/" + x).iloc[:, :35].assign(network=x)) for x in csvs]
    df_concat = pd.concat(list_dfs)
    bool_df = df_concat.replace('open', False).replace('closed', True)
    vals = np.array([bool_df.loc[bool_df['breaker'] == x, [str(y) for y in range(-2, 32)]].sum() > 0 for x in
    ...
    df_vals.columns = bool_df.iloc[:, :35].columns
